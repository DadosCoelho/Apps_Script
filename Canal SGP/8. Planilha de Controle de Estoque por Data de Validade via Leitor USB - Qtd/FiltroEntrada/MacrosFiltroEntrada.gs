...
function dadosFiltroEntrada(){
  ...
  for(var i = 0; i < dadosEntrada.length; i++){
    ...
    var DataValidade = Utilities.formatDate(new Date(dadosEntrada[i][4]), planilha.getSpreadsheetTimeZone(),"dd/MM/yyyy");

    dadosEntrada[i][4] = DataValidade;

    var Data = Utilities.formatDate(new Date(dadosEntrada[i][5]), planilha.getSpreadsheetTimeZone(),"dd/MM/yyyy");

    dadosEntrada[i][5] = Data;

    if(dadosEntrada[i][6] != ""){
      var Hora = dadosEntrada[i][6].getHours().toString().padStart(2, '0');
      var Minutos = dadosEntrada[i][6].getMinutes().toString().padStart(2, '0');
      var Segundos = dadosEntrada[i][6].getSeconds().toString().padStart(2, '0');
      var Hora =  Hora + ":" + Minutos + ":" + Segundos;
      dadosEntrada[i][6] = Hora;
    }
