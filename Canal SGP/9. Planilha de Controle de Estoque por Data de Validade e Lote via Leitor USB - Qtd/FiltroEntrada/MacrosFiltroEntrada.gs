...
function dadosFiltroEntrada(){
  ...
  var dadosEntrada = guiaEntrada.getRange(2, 1, ultimaLinha, 8).getValues();
  ...
  for(var i = 0; i < dadosEntrada.length; i++){
    ...
    if(dadosEntrada[i][7] != ""){
      var Hora = dadosEntrada[i][7].getHours().toString().padStart(2, '0');
      var Minutos = dadosEntrada[i][7].getMinutes().toString().padStart(2, '0');
      var Segundos = dadosEntrada[i][7].getSeconds().toString().padStart(2, '0');
      var Hora =  Hora + ":" + Minutos + ":" + Segundos;
      dadosEntrada[i][7] = Hora;
    }
