function FormFiltro() {
  ...
  var Form = HtmlService.createTemplateFromFile("Filtro");
...
function dadosFiltroContagem(){
  ...
  var dadosContagem = guiaContagem.getRange(2, 1, ultimaLinha, 7).getValues();
  ...
  for(var i = 0; i < dadosContagem.length; i++){
    ...
    if(dadosContagem[i][6] != ""){
      var Hora = dadosContagem[i][6].getHours().toString().padStart(2, '0');
      var Minutos = dadosContagem[i][6].getMinutes().toString().padStart(2, '0');
      var Segundos = dadosContagem[i][6].getSeconds().toString().padStart(2, '0');
      var Hora =  Hora + ":" + Minutos + ":" + Segundos;
      dadosContagem[i][6] = Hora;
    }
